...
def _rotate_match_coords(
    ...
    """
    Inverse-rotate OCR bounding-box coordinates from the corrected (upright)
    image space back to the original (pre-rotation) image/PDF space.

    When we rotated the image by `rotation` degrees to fix its orientation,
    the OCR engine sees the upright image and returns coordinates in that space.
    But the PDF page still uses the original coordinate system, so we must
    reverse-transform the coordinates before placing PDF annotations.

    rotation: angle IN DEGREES that was applied to the image to correct it
              (e.g. 90 means the image was rotated 90° clockwise).
    img_w / img_h: width & height of the CORRECTED image (i.e. after rotation).
    """
    result = []
    for m in matches:
        ...
        if rotation == 90:
            # corrected image: (x, y) -> original: (y, W - x)
            # where W is the width of the corrected image
            nx1 = y1
            ny1 = img_w - x2
            nx2 = y2
            ny2 = img_w - x1
            ...
            # corrected image: (x, y) -> original: (H - y, x)
            # where H is the height of the corrected image
            nx1 = img_h - y2
            ny1 = x1
            nx2 = img_h - y1
            ny2 = x2
        ...
        nm = dict(m)
        nm["x1"], nm["y1"], nm["x2"], nm["y2"] = nx1, ny1, nx2, ny2
        result.append(nm)
    return result
...
def process_pdf(
    ...
    for page_idx, page in enumerate(doc, start=1):
        ...
        page_results = []
        rotation_applied = 0   # only set for scanned pages
        img_path = ""          # only set for scanned pages
        ...
        if use_native:
            ...
            # Step 1: Detect & correct orientation FIRST (overwrites img_path in-place).
            # This ensures OCR always sees an upright image, so its coordinates
            # are relative to the corrected (upright) image.
            rotation_applied = detect_and_correct_orientation(img_path)

            # Step 2: Run OCR on the corrected image.
        ...
        if matches:
            ...
            if do_highlight:
                if use_native:
                    ...
                    # OCR coordinates are from the CORRECTED (upright) image.
                    # The PDF page still has its original (possibly rotated) coordinate
                    # space, so we must inverse-transform coords before annotating.
                    if not use_native and rotation_applied != 0:
                        from PIL import Image as _PIL_Image
                        with _PIL_Image.open(img_path) as _im:
                            _cw, _ch = _im.size   # corrected image dimensions
                        pdf_matches_coords = _rotate_match_coords(
                            matches, rotation_applied, _cw, _ch
                        )
                    else:
                        pdf_matches_coords = matches
                    highlight_page_by_coords(page, pdf_matches_coords, scale_ocr)
                
            # 4. Also highlight the page image if it was scanned.
            # img_path already points to the corrected image, and OCR coords
            # are relative to that image, so no transformation is needed here.
            if not use_native:
